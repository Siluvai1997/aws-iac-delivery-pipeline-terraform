...
resource "aws_codepipeline" "tf_codepipeline" {
  ...
  stage {
    ...
    action {
      ...
      configuration = {
        ProjectName = var.codebuild_terraform_validation_name
      }
    }
  }
...
resource "aws_codestarnotifications_notification_rule" "pipeline_rule" {
  ...
  name     = "Tf-codepipeline-notification"
  resource = aws_codepipeline.tf_codepipeline.arn
